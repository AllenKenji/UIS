...
import { ResidentsAPI, ComplaintsAPI, BusinessesAPI, IncidentsAPI, api } from "../services/api";
import "./resident.css";
  ...
  useEffect(() => {
    const fetchRecords = async () => {
      ...
      try {
        ...
        const [allBusinesses, docsRes, complaintsRes, allIncidents] = await Promise.all([
          BusinessesAPI.listAll().catch(() => []),
          api.get("/api/documents/my", { params: { resident_id: residentId } }).catch(() => ({ data: [] })),
          ComplaintsAPI.listMine().catch(() => []),
          IncidentsAPI.list().catch(() => []),
        ]);

        setBusinesses((Array.isArray(allBusinesses) ? allBusinesses : []).filter((business) =>
          business.ownerUid === residentId || (resident?.email && business.email === resident.email)
        ));
        ...
        const incidentMap = {};
        (Array.isArray(allIncidents) ? allIncidents : [])
          .filter((incident) => incident.residentId === residentId || incident.authUid === residentId)
          .forEach((incident) => { incidentMap[incident.id] = incident; });
