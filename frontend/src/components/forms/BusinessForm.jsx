...
const BusinessForm = ({ onBusinessAdded, onCancel, residentProfile }) => {
  ...
  const uploadAllDocuments = async (businessName) => {
    ...
    for (const key of Object.keys(documents)) {
      ...
      uploaded[key] = await uploadLocalFile(user.uid, compressed, path, safeFileName);
    }
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const payload = {
        owner_uid: user.uid,
        owner_name: user.fullName || user.full_name || user.email,
        ...
        documents: {
          valid_id: uploadedDocs.validId?.url || "",
          proof_of_address: uploadedDocs.proofOfAddress?.url || "",
          dti_cert: uploadedDocs.dtiCert?.url || null,
          business_logo: uploadedDocs.businessLogo?.url || null,
        },
      };

      await BusinessesAPI.createApplication(payload);
